...
| # | Feature | Description |
|---|---------|-------------|
| F-1 | **OCR Agent** | Extract raw text from PDFs (text-based + scanned) and images (PNG, JPEG) using Tesseract/PyMuPDF; fallback to GPT-4o vision for images |
| F-2 | **Extraction Agent** | LLM-powered (GPT-4o, structured output) extraction of invoice fields from raw text: invoice_number, date, due_date, vendor, buyer, line_items, subtotal, tax, total, currency |
...
### Integration
- OpenAI API key available (GPT-4o access)
- Internet connectivity for LLM API calls
- No external system integrations

### LLM
- GPT-4o with structured output (JSON mode) for extraction
- GPT-4o vision for image-based fallback OCR
